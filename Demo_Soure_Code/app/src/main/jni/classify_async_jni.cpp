    ...
    const AiContext& context, int result, const vector<shared_ptr<AiTensor>>& outputTensor, int32_t istamp)
{
    ...
    if (result != 0) {
        LOGI("[AI_DEMO_ASYNC] AYSNC infrence error is %d.", result);
        return;
    }
    gettimeofday(&tpend, nullptr);
    time_use = 1000000 * (tpend.tv_sec - tpstart.tv_sec) + tpend.tv_usec - tpstart.tv_usec;
    LOGI("[AI_DEMO_ASYNC] AYSNC inference time %f ms, JNI layer onRunDone istamp: %d", time_use / 1000, istamp);
    JNIEnv* env = nullptr;
    ...
    for (auto key : keys) {
        ...
        LOGI("[AI_DEMO_ASYNC] key: %s, value: %s.", key.c_str(), value.c_str());
    }
    ...
    vector<int>::size_type outputTensorSize = outputTensor.size();
    for (vector<int>::size_type j = 0; j < outputTensorSize; j++) {
    ...
    if (onValueReceived == nullptr) {
        LOGI("[AI_DEMO_ASYNC] jni onValueReceived null");
    }
...
{
    LOGE("[AI_DEMO_ASYNC] JNI layer OnServiceDied:");
    JNIEnv* env = nullptr;
    jvm->AttachCurrentThread(&env, nullptr);
...
void AsyncResourceDestroy(shared_ptr<AiModelBuilder>& modelBuilder, vector<MemBuffer*>& memBuffers)
{
    if (modelBuilder == nullptr) {
        LOGE("[AI_DEMO_ASYNC] modelBuilder is null.");
        return;
    }
...
int LoadASync(vector<string>& names, vector<string>& modelPaths, shared_ptr<AiModelMngerClient>& client)
{
    ...
    if (modelBuilder == nullptr) {
        LOGE("[AI_DEMO_ASYNC] creat mcbuilder failed.");
        return FAILED;
    }
    ...
    for (size_t i = 0; i < modelPaths.size(); ++i) {
        ...
        LOGI("[AI_DEMO_ASYNC] modelpath is %s\n.", modelPath.c_str());
        MemBuffer* buffer = modelBuilder->InputMemBufferCreate(modelPath);
        if (buffer == nullptr) {
            LOGE("[AI_DEMO_ASYNC] cannot find the model file.");
            return FAILED;
        }
        memBuffers.push_back(buffer);
        string modelNameFull = string(modelName) + string(".om");
        shared_ptr<AiModelDescription> desc = make_shared<AiModelDescription>(modelNameFull, 3, 0, 0, 0);
        if (desc == nullptr) {
            LOGE("[AI_DEMO_ASYNC] LoadASync: desc make_shared error.");
            AsyncResourceDestroy(modelBuilder, memBuffers);
            return FAILED;
        }
        desc->SetModelBuffer(buffer->GetMemBufferData(), buffer->GetMemBufferSize());

        LOGE("[AI_DEMO_ASYNC] loadModel %s IO Tensor.", desc->GetName().c_str());
    ...
    AsyncResourceDestroy(modelBuilder, memBuffers);
    if (ret != 0) {
        LOGE("[AI_DEMO_ASYNC] Model Load Failed.");
        return FAILED;
    }
    return SUCCESS;
}
...
int UpdateAsyncInputTensorVec(vector<TensorDimension>& inputDims, bool isUseAipp, string& modelName)
{
    ...
    for (auto inDim : inputDims) {
        ...
        if (isUseAipp) {
            ...
            LOGI("[AI_DEMO_ASYNC] model %s uses AIPP(input1).", modelName.c_str());
        } else {
            ret = input->Init(&inDim);
            LOGI("[AI_DEMO_ASYNC] model %s does not use AIPP(input1).", modelName.c_str());
        }
        if (ret != 0) {
            LOGE("[AI_DEMO_ASYNC] model %s AiTensor Init failed(input1).", modelName.c_str());
            return FAILED;
        }
    ...
    if (input_tensor1_vec.size() == 0) {
        LOGE("[AI_DEMO_ASYNC] input_tensor1_vec.size() == 0");
        return FAILED;
    }
    LOGE("[AI_DEMO_ASYNC] input_tensor1_vec.size %zu ", input_tensor1_vec.size());
    // input 2
    for (auto inDim : inputDims) {
        shared_ptr<AiTensor> input = make_shared<AiTensor>();
        if (isUseAipp) {
            ret = input->Init(inDim.GetNumber(), inDim.GetHeight(), inDim.GetWidth(), AiTensorImage_YUV420SP_U8);
            LOGI("[AI_DEMO_ASYNC] model %s uses AIPP(input2).", modelName.c_str());
        } else {
            ret = input->Init(&inDim);
            LOGI("[AI_DEMO_ASYNC] model %s does not use AIPP(input2).", modelName.c_str());
        }
        if (ret != 0) {
            LOGE("[AI_DEMO_ASYNC] model %s AiTensor Init failed(input2).", modelName.c_str());
            return FAILED;
        }
    ...
    if (input_tensor2_vec.size() == 0) {
        LOGE("[AI_DEMO_ASYNC] input_tensor2_vec.size() == 0");
        return FAILED;
    }
    LOGE("[AI_DEMO_ASYNC] input_tensor2_vec.size %zu ", input_tensor2_vec.size());
    return SUCCESS;
}
...
int UpdateAsyncOutputTensorVec(vector<TensorDimension>& outputDims, string& modelName)
{
    ...
    for (auto out_dim : outputDims) {
        ...
        if (ret != 0) {
            LOGE("[AI_DEMO_ASYNC] model %s AiTensor Init failed(output).", modelName.c_str());
            return FAILED;
        }
    ...
    if (output_tensor_vec.size() == 0) {
        LOGE("[AI_DEMO_ASYNC] output_tensor_vec.size() == 0");
        return FAILED;
    }
    return SUCCESS;
}
...
shared_ptr<AiModelMngerClient> LoadModelASync(vector<string> names, vector<string> modelPaths, vector<bool> Aipps)
{
    ...
    IF_BOOL_EXEC(ptrClient == nullptr, LOGE("[AI_DEMO_ASYNC] Model Manager Client make_shared error.");
                 return nullptr);
    int ret = ptrClient->Init(listener);
    IF_BOOL_EXEC(ret != SUCCESS, LOGE("[AI_DEMO_ASYNC] Model Manager Init Failed."); return nullptr);
    ret = LoadASync(names, modelPaths, ptrClient);
    IF_BOOL_EXEC(ret != SUCCESS, LOGE("[AI_DEMO_ASYNC] LoadASync Failed."); return nullptr);
    ...
    for (size_t i = 0; i < names.size(); ++i) {
        ...
        LOGI("[AI_DEMO_ASYNC] Get model %s IO Tensor. Use AIPP %d", modelName.c_str(), isUseAipp);
        ...
        IF_BOOL_EXEC(ret != SUCCESS, LOGE("[AI_DEMO_ASYNC] Get Model IO Tensor Dimension failed,ret is %d.", ret);
                     return nullptr);
        IF_BOOL_EXEC(inputDims.size() == 0, LOGE("[AI_DEMO_ASYNC] inputDims.size() == 0"); return nullptr);
    ...
    JNIEnv* env, jclass type, jobject modelInfo)
{
    ...
    IF_BOOL_EXEC(classList == nullptr, LOGE("[AI_DEMO_ASYNC] can not find List class."); return nullptr);
    ...
    for (int i = 0; i < len; i++) {
        ...
        IF_BOOL_EXEC(getOfflineModelName == nullptr, LOGE("[AI_DEMO_ASYNC] can not find getOfflineModelName method.");
                     return nullptr);
        IF_BOOL_EXEC(getModelPath == nullptr, LOGE("[AI_DEMO_ASYNC] can not find getModelPath method.");
                     return nullptr);
        IF_BOOL_EXEC(getUseAIPP == nullptr, LOGE("[AI_DEMO_ASYNC] can not find getUseAIPP method."); return nullptr);
        ...
        LOGI("[AI_DEMO_ASYNC] modelName is %s .", modelName);
        IF_BOOL_EXEC(modelName == nullptr, LOGE("[AI_DEMO_ASYNC] modelName is invalid."); return nullptr);
        const char* modelPath = env->GetStringUTFChars(modelpath, 0);
        IF_BOOL_EXEC(modelPath == nullptr, LOGE("[AI_DEMO_ASYNC] modelPath is invalid."); return nullptr);
        LOGI("[AI_DEMO_ASYNC] useaipp is %d.", bool(useaipp == JNI_TRUE));
    ...
    if (!mclientAsync) {
        ...
        IF_BOOL_EXEC(mclientAsync == nullptr, LOGE("[AI_DEMO_ASYNC] mclientAsync loadModel is nullptr.");
                     return nullptr);
    }
    // load model
    LOGI("[AI_DEMO_ASYNC] sync load model INPUT NCHW : %d %d %d %d.", inputDimension[0][0].GetNumber(),
        inputDimension[0][0].GetChannel(), inputDimension[0][0].GetHeight(), inputDimension[0][0].GetWidth());
    LOGI("[AI_DEMO_ASYNC] sync load model OUTPUT NCHW : %d %d %d %d.", outputDimension[0][0].GetNumber(),
...
int runAsyncProcess(
    JNIEnv* env, jobject bufList, jmethodID listGet, int vecIndex, int listLength, const char* modelName)
{
    auto input_tensor0 = findInputTensor(vecIndex);
    if (input_tensor0 == nullptr) {
        LOGE("[AI_DEMO_ASYNC] findInputTensor is nullptr.");
        return FAILED;
    }
    for (int i = 0; i < listLength; i++) {
        if (static_cast<vector<int>::size_type>(i) >= (*input_tensor0).size()) {
            break;
        }
        jbyteArray buf_ = (jbyteArray)(env->CallObjectMethod(bufList, listGet, i));
        IF_BOOL_EXEC(buf_ == nullptr, LOGE("[AI_DEMO_ASYNC] buf_ is nullptr."); return FAILED);
        ...
        if (((*input_tensor0)[i]->GetSize() != databuffsize)) {
            LOGE("[AI_DEMO_ASYNC] input->GetSize(%d) != databuffsize(%d) ", (*input_tensor0)[i]->GetSize(),
                databuffsize);
            return FAILED;
        }
    ...
    LOGI("[AI_DEMO_ASYNC] JNI runModel modelname:%s", value.c_str());

    int istamp = 0;
    gettimeofday(&tpstart, nullptr);
    int ret = mclientAsync->Process(context, *input_tensor0, output_tensor_vec[vecIndex], 300, istamp); // 300: timeout
    IF_BOOL_EXEC(ret != SUCCESS, LOGE("[AI_DEMO_ASYNC] Runmodel Failed! ret=%d.", ret); return FAILED);
    LOGI("[AI_DEMO_ASYNC] Runmodel Succ! istamp=%d.", istamp);
...
{
    IF_BOOL_EXEC(env == nullptr, LOGE("[AI_DEMO_ASYNC] runModelAsync env is null"); return);
    jclass ModelInfo = env->GetObjectClass(modelInfo);
    IF_BOOL_EXEC(ModelInfo == nullptr, LOGE("[AI_DEMO_ASYNC] can not find ModelInfo class."); return);
    IF_BOOL_EXEC(bufList == nullptr, LOGE("[AI_DEMO_ASYNC] bufList is null."); return);
    ...
    IF_BOOL_EXEC(getOfflineModelName == nullptr, LOGE("[AI_DEMO_ASYNC] can not find getOfflineModelName method.");
                 return);
    IF_BOOL_EXEC(getModelPath == nullptr, LOGE("[AI_DEMO_ASYNC] can not find getModelPath method."); return);
    ...
    const char* modelName = env->GetStringUTFChars(modelname, 0);
    IF_BOOL_EXEC(modelName == nullptr, LOGE("[AI_DEMO_ASYNC] modelName is invalid."); return);
    int vecIndex = g_asyncNameToIndex[modelName];
    const char* modelPath = env->GetStringUTFChars(modelpath, 0);
    IF_BOOL_EXEC(modelPath == nullptr, LOGE("[AI_DEMO_ASYNC] modelPath is invalid."); return);
    // buf_list
    jclass classList = env->GetObjectClass(bufList);
    IF_BOOL_EXEC(classList == nullptr, LOGE("[AI_DEMO_ASYNC] can not find List class."); return);
    // method in class
    jmethodID listGet = env->GetMethodID(classList, "get", "(I)Ljava/lang/Object;");
    jmethodID listSize = env->GetMethodID(classList, "size", "()I");
    IF_BOOL_EXEC(listGet == nullptr, LOGE("[AI_DEMO_ASYNC] can not find get method."); return);
    IF_BOOL_EXEC(listSize == nullptr, LOGE("[AI_DEMO_ASYNC] can not find size method."); return);
    int listLength = static_cast<int>(env->CallIntMethod(bufList, listSize));
    IF_BOOL_EXEC(listLength > 0, LOGI("[AI_DEMO_ASYNC] input data length is %d .", listLength));
    callbacksInstance = env->NewGlobalRef(callbacks);
    jclass objClass = env->GetObjectClass(callbacks);
    IF_BOOL_EXEC(!objClass, LOGE("[AI_DEMO_ASYNC] objClass is nullptr."); return);
    env->GetJavaVM(&jvm);
    callbacksClass = reinterpret_cast<jclass>(env->NewGlobalRef(objClass));
    env->DeleteLocalRef(objClass);
    // load
    IF_BOOL_EXEC(!mclientAsync, LOGE("[AI_DEMO_ASYNC] mclientAsync is nullptr."); return);
    env->ReleaseStringUTFChars(modelpath, modelPath);
    // run
    LOGI("[AI_DEMO_ASYNC] INPUT NCHW : %d %d %d %d.", inputDimension[0][0].GetNumber(),
        inputDimension[0][0].GetChannel(), inputDimension[0][0].GetHeight(), inputDimension[0][0].GetWidth());
    LOGI("[AI_DEMO_ASYNC] OUTPUT NCHW : %d %d %d %d.", outputDimension[0][0].GetNumber(),
        outputDimension[0][0].GetChannel(), outputDimension[0][0].GetHeight(), outputDimension[0][0].GetWidth());
